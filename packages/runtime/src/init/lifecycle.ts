...
const callUserLifecycleFunction = function(vm, func, args) {
  if (isFunction(func)) {
    return func.apply(vm, args);
  }

  return func.map(x => x.apply(vm, args))[func.length - 1];
};

export const patchAppLifecycle = vm => {
  const patchedOptions = {
    onLaunch(...args) {
      vm.$wx = this;
      this.$wepy = vm;

      const { onLaunch } = vm.$options;

      if (onLaunch) {
        return callUserLifecycleFunction(vm, onLaunch, args);
      }
    }
  };

  for (const lifecycleName of WEAPP_APP_LIFECYCLE) {
    const func = vm.$options[lifecycleName];

    if (!patchedOptions[lifecycleName] && func) {
      patchedOptions[lifecycleName] = function(...args) {
        return callUserLifecycleFunction(vm, func, args);
      };
    }
  }
